    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            ...
            if noisyDistance is None:
                ...
                allPossible[jailpos]=1 #yendigi zaman nerede oldugunu biliyorum
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            newGameState=self.setGhostPosition(gameState,p) #update game state
            newPosDist = self.getPositionDistribution(newGameState)
            #print newPosDist
            for newPosition, distribution in newPosDist.items():
                #print allPossible[newPosition]
                allPossible[newPosition]=allPossible[newPosition]+self.beliefs[p]*distribution
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        #print positions
        numberOfLegalPositions=len(positions)

        print numberOfParticles
        print numberOfLegalPositions

        division=numberOfParticles/numberOfLegalPositions
        print division

        for i in range(0,numberOfParticles):
            #uniformLocation=i/division-1
            #print uniformLocation
            uniformLocation=i/division
            if(uniformLocation>=numberOfLegalPositions):
                particles.append(positions[random.randint(0,numberOfLegalPositions-1)])
            else:
                particles.append(positions[int(uniformLocation)])
    ...
    def observe(self, observation, gameState):
        ...
        oldBeliefDist = self.getBeliefDistribution()


        numberParticiles=self.numParticles
        particles = []

        if noisyDistance is None:

            for i in range(0,numberParticiles):
                particles.append(self.getJailPosition())
            ...
            for p in self.particles:
                #print oldBeliefDist[p]
                tempDistance=util.manhattanDistance(p,pacmanPosition)
                allPossible[p]=oldBeliefDist[p]*emissionModel[tempDistance]
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            ...
            tempDistribution = self.getPositionDistribution(self.setGhostPosition(gameState, p))

            sample=util.sample(tempDistribution)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        cartesian=list(itertools.product(positions, repeat=numberOfGhosts))

        random.shuffle(cartesian)
        print cartesian
        division=numberOfParticles

        for i in range(0,numberOfParticles):
            particles.append(cartesian[i%len(cartesian)])
    ...
    def observeState(self, gameState):
        ...
        for i in range(self.numGhosts):
            ...
            if noisyDistances[i] is None:
                ...
                for p in self.particles:
                    partici=self.getParticleWithGhostInJail(p, i)
                    particles.append(partici)
                ...
                for p in self.particles:
                    # print p
                    tempDistance = util.manhattanDistance(p[i], pacmanPosition)
                    allPossible[p] = oldBeliefDist[p]* emissionModels[i][tempDistance]

                # print allPossible
                # allPossible.normalize()
                sum = 0
                for j in allPossible:
                    sum = sum + allPossible[j]
                ...
                if (sum == 0):
                    ...
                    for j in range(0, numberParticiles):
                        particles.append(util.sample(allPossible))
    ...
    def getBeliefDistribution(self):
        ...
        for particle in particles:
            beliefDist[particle]=beliefDist[particle]+1.0
